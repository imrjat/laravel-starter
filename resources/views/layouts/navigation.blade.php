...
@if(auth()->user()->isOwner())
    @if(auth()->user()->tenant->isOnTrial())
        <x-a href="{{ route('admin.billing') }}" class="my-5 btn btn-primary w-full text-center">{{ __("Trial ends in ").auth()->user()->tenant->trial_ends_at->diffInDays().__(" days, Subscribe") }}</x-a>
    @endif

    @if(auth()->user()->tenant->isOnGracePeriod())
        <x-a href="{{ route('admin.billing') }}" class="my-5 btn btn-primary w-full text-center">{{ __("Subscription will end in ").auth()->user()->tenant->ends_at->diffInDays().__(" days, Resume Subscription") }}</x-a>
    @endif
@endif
